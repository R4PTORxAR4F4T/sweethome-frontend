...
const Navbar = () => {
    ...
    const handleLogout = async () => {
        try {
            await axios.post('http://localhost:4000/control/logout', {}, { withCredentials: true });
            router.push('/login');
        } catch (error) {
    ...
    return (
        <div className='sticky z-10'>
            ...
            <a href="" className="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-green-400">Sweet Home</span>
            </a>
            <div className="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                <button type="button" className="flex text-sm bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
                    <span className="sr-only">Open user menu</span>
                    <img className="w-8 h-8 rounded-full" src="../globe.svg"/>
                </button>
